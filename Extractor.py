...
class DataExtractor:
    ...
    def extract_experience(self):
        ...
        for token in self.doc:
            if token.text in RESUME_SECTIONS:
                if token.text.lower() in ['experience', 'experience','technical experience', 'professional experience', 'work experience']:
    ...
    def extract_skills(self):
        """
        Extract the skills section from the given resume text.

        Args:
        - text (str): The input text.

        Returns:
        - str: The extracted skills.
        """
        ...
        for token in self.doc:
            if token.text in RESUME_SECTIONS:
                if token.text.lower() in ['skills','technical skills']:
    ...
    def extract_position_year(self):
        ...
        """
        position_year_search_pattern = r"\b\d{4}\s*-\s*(?:\d{4}|\bpresent\b)|\b[A-Za-z]{3}\s\d{4}\s*–\s*\b[A-Za-z]{3}\s\d{4}"
    ...
    def extract_colleges(self):
        colleges_section = []
        in_colleges_section = False

        for token in self.doc:
            
            if token.text in RESUME_SECTIONS:
                if token.text.lower() in ['education','examination']:
                    in_colleges_section = True
                    # print(token.text)
                else:                                                                                        
                    # if in_skills_section:
                    #       break
            ...
            if in_colleges_section:
                colleges_section.append(token.text)

        colleges_section=' '.join(colleges_section)
        


        text = colleges_section
        print(colleges_section)
        lines = text.split('\n')
        pattern = r"(?i).*\b(?:IIT|School|College|IIT|University)\b.*"
        ...
        for text in matches:
            ...
            for ent in entities:
                ...
                university_names.append([ent for ent in entities if ent.label == 383])
